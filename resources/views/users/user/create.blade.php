...
@section('content')
<div class="container-fluid">
    <form action="@if(isset($data)) {{ route('users.update', $data->id) }} @else {{ route('users.store') }} @endif" method="POST" enctype="multipart/form-data">
    @csrf
    @if (isset($data))
        @method("PUT")
    @endif
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <div class="custom-control custom-checkbox">
                        <input class="custom-control-input" name="roles[]" type="checkbox" id="roleCheckbox{{$key}}" @if(old('roles')) @foreach(old('roles') as $val) @if($val == $value->id) checked @endif @endforeach @elseif(isset($data)) @foreach($data->roles as $val) @if($val->name == $value->name) checked @endif @endforeach @elseif ($value->name == 'user') checked @endif value="{{ $value->id }}">
                        <label for="roleCheckbox{{$key}}" class="custom-control-label">{{ $value->name }}</label>
                    </div>
                    @endforeach
                    @endif
                </div>
